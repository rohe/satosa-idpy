...
class TestComboCollect(object):
    ...
    def setup(self):
        ...
        try:
            shutil.rmtree(STORE_CONF["kwargs"]["fdir"])
        ...
        self.op["federation_entity"].persistence = FEPersistence(
            storage, upstream_get=self.op["federation_entity"].unit_get)
    ...
    def test_collect_trust_chain(self):
        ...
        #
        self.op["federation_entity"].persistence.reset_state()
        #
        self.op["federation_entity"].persistence.restore_state()

        assert self.op["federation_entity"].trust_chain
